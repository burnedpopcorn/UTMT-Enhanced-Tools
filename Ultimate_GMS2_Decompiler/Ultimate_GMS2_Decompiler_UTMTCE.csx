/*
	Ultimate_GMS2_Decompiler_UTMTCE
	
	Original Decompiler by loypoll
	Fixed and Improved by burnedpopcorn180
	
	Ultimate_GMS2_Decompiler_UTMTCE Improvements include:
	...
	GMS2_Decompiler_FIXED_UTMT Improvements include:
		...
		foreach (var j in obj.Events[i])
		{
			...
			else
			{
				...
				if (j.Actions.Count > 0)
				{
					...
					File.WriteAllText(rootPath + objectPath + codeName + ".gml", code);// dont do replacewholeword here
				}
...
void DumpRoom(UndertaleRoom room)
{
	...
	if (room.CreationCodeId != null && !COMP)
	{
		...
		File.WriteAllText(rootPath + roomPath + "RoomCreationCode.gml", code.ReplaceWholeWord("states.parry", "states.parry_").ReplaceWholeWord("states.throw", "states.throw_"));
...
async Task DumpScripts()
{
	...
	if (CSTM.Count == 0)
	{
		...
		if (code != "")
		{
			...
			// finish																	This is to fix Enums from breaking across the Project \\
			File.WriteAllText(rootPath + scriptPath + globalinit.name + ".gml", code.ReplaceWholeWord("states.parry", "states.parry_").ReplaceWholeWord("states.throw", "states.throw_"));
...
string decompileCode(UndertaleCode codeId)
{
	...
	if (!code.EndsWith("\n"))
		code += "\n";
	return code.ReplaceWholeWord("states.parry", "states.parry_").ReplaceWholeWord("states.throw", "states.throw_");
}
...
{
	AutoSize = true,
	Text = "Ultimate_GMS2_Decompiler_UTMTCE",
	MaximizeBox = false,
	MinimizeBox = false,
...
if (!GMS1)
{
	...
	if (_PJCT.Checked && SCPT)
	{
		...
		var str = @"Project Decompilied by Ultimate_GMS2_Decompiler_UTMTCE.csx
	Improved by burnedpopcorn180
		Original Version by loypoll
		
WARNING!

	states.parry and states.throw have been automatically changed to
	states.parry_ and states.throw_ because GameMaker would think you were trying to call
	parry() and throw() functions otherwise
	
	This has been applied to all Scripts, Objects, and Rooms, but still make sure that no instances
	of states.parry and states.throw still exist
	
	You can globally check by pressing CTRL+SHIFT+F and searching for them



As a warning, neither this Decompiler nor UTMTCE are perfect.
...
Assets:
	...
	This isn't something this Script can (reliability) fix, as UTMTCE would have to Guess what are Asset IDs and what are just Numbers
	To Fix this, reference the Included 'Asset_Order' Note, as that provides a List of Asset IDs and their respective Asset
	
Enums:

	The Enum Declaration have been Extracted for you in the 'GLOBALINIT' Script
	So no further action is required
	But you should at least try to move it into a more suitable place
...
";
		...
		if (Directory.Exists($"{dataPath}code"))
			...
			str += "\nAlso since this game is made in GM2.3+, some Code might have Failed to Decompile.\nIt's the fault of UTMTCE. Just fix it yourself, or maybe try UnderAnalyzer and see if that can Decompile that specific script\n";
...
if (_PJCT.Checked && SCPT) {
	...
	//										To Fix Bunch o' errors with UTMTCE's Enums
	File.AppendAllText(enumPath, enum_code.ReplaceWholeWord("parry", "parry_").ReplaceWholeWord("throw", "throw_"));
	
	// This was done for decompiling Scripts, Objects, and Rooms
	// code.ReplaceWholeWord("states.parry", "states.parry_").ReplaceWholeWord("states.throw", "states.throw_")
	} catch (Exception e) {
		// Fucking AntonBall: WHY DOES IT HAVE NO SCRIPTS???
		errorList.Add("GLOBALINIT - Failed to Apply Enum Declaration in GLOBALINIT Script (are scripts present in the data.win?)");
	}
}
...
{
	writer.WriteLine("Generated by Ultimate_GMS2_Decompiler_UTMTCE.csx");
	writer.WriteLine("");
